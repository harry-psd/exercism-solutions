...
def add_item(current_cart, items_to_add):
    ...
    for new_item in items_to_add:
        current_cart[new_item] = current_cart.setdefault(new_item, 0) + 1
...
def read_notes(notes):
    ...
    """
    new_user_cart = dict.fromkeys(notes, 1)
...
def send_to_store(cart, aisle_mapping):
    ...
    for item in cart.keys():
        ...
        fulfillment_cart[item].append(cart[item])
...
def update_store_inventory(fulfillment_cart, store_inventory):
    ...
    for item in fulfillment_cart.keys():
        quantity = fulfillment_cart[item][0]
        if store_inventory[item][0] <= quantity:
